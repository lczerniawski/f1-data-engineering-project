...
races_df = (
    spark.read.parquet(f"{processed_folder_path}/races")
...
results_df = (
    spark.read.parquet(f"{processed_folder_path}/results")
        .withColumnRenamed("time", "race_time")
        .withColumnRenamed("date", "race_date")
        .drop("ingestion_date")
        .drop("race_name")
...
race_results_df = (
    results_df
    .join(races_df, (results_df.round == races_df.round) & (results_df.season == races_df.race_season), how="inner")
    .join(circuits_df, circuits_df.circuit_id ==races_df.circuit_id, how="inner")
    .join(drivers_df, drivers_df.driver_id == results_df.driver_id, how="inner")
    .join(constructors_df, results_df.constructor_id == constructors_df.constructor_id, how="inner")
)

final_df = (
    race_results_df
        .select("race_season", "race_name", "race_date", "locality", "driver_name", "driver_family_name", "permanent_number", "driver_nationality",
